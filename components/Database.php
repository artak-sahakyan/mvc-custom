...
class Database
{
     ...
     * @return array recordset
     */
    public function fetch_all($table, $offset, $perPage, $sortQuery)
    {
        $sel = $this->pdo->prepare("SELECT * FROM $table $sortQuery LIMIT $offset, $perPage");
        $sel->execute();
        $sel->setFetchMode(PDO::FETCH_OBJ);
        return $sel->fetchAll();
    }
    ...
    {
        return $this->pdo->query("SELECT COUNT(*) as total FROM $table")->fetchColumn();
     ...
     */
    public function update($table, $dat, $primaryKeyValue, $primaryKey = 'id')
    {
        if ($dat !== null)
            $data = array_values($dat);
        array_push($data, $primaryKeyValue);
        //grab keys
        $cols = array_keys($dat);
        $mark = array();
        foreach ($cols as $col) {
        ...
        $im = implode(', ', $mark);
        $ins = $this->pdo->prepare("UPDATE $table SET $im where $primaryKey=?");
        $ins->execute($data);
    }
